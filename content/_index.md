---
title: Capture The MITRE ATT&CK Flag
weight: 1
archetype: home
---

## Workshop Guide

**Written By:** Jason Palm, Kyle Krisher, & Jason Graun

![alt text](Threat-Detection-and-Response.png?width=400px)

FortiEDR & FortiAnalyzer work together to give SOC analysts the visibility and tools needed for effective threat hunting. FortiEDR provides real-time endpoint telemetry, behavioral monitoring, and forensic timelines, allowing analysts to investigate IOCs, detect suspicious activity, and contain compromised hosts. FortiAnalyzer centralizes logs from across the network, correlates events, integrates threat intelligence, and offers advanced querying and dashboards aligned to MITRE ATT&CK. Together, they enable analysts to pivot between endpoint and network data, spot multi-vector attacks, and proactively hunt for threats before they escalate.

In this hands-on workshop you will step into the role of a SOC Analyst where you’ll learn how to hunt for threats using FortiEDR and FortiAnalyzer. You will gain practical experience mapping adversary tactics and techniques to the MITRE ATT&CK framework while investigating a live attack scenario. Throughout the session, you will uncover "flags" hidden in the environment, answer guided questions, and score points as you progress—turning learning into a competitive, interactive challenge.

## Images

There are numerous images used in this workshop to help direct you and provide further information. Some images contain numerous details. Please note that any image can be enlarged by simply clicking on it. For example, try clicking on the image below to see where FortiEDR fits within the Fortinet product portfolio in relation to breaking an attacker's sequence.

![](attack_sequence.JPG?width=500px)